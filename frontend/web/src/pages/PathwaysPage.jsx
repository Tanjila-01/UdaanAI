...
import { getPathwaysApi, getPathwayDetailApi } from '../api/client';
...
const PathwayDetailView = ({ detail, loading, error, onRetry }) => {
  if (loading) {
    return (
      <div className="bg-white border border-slate-200 rounded-3xl p-8 space-y-4 animate-pulse">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-3">
          {detail.options?.map((opt) => (
            <div 
              key={opt.id}
              className="bg-[#F8FAF8] border border-slate-200/80 rounded-2xl p-4 space-y-1.5"
            >
              <div className="flex items-center justify-between">
