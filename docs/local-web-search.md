...
A maximum of 64 impersonal answers are cached in process for five minutes. Cache hits preserve the actual original checked_at time. Obvious personal queries are excluded. The cache is not a persistent knowledge database; service restarts clear it. Rate capacity is shared by cloud generation and local voice transcription.
...
Across earlier live evaluations, fresh questions took about 6-70 seconds on this machine; a repeated cached AI question took 0.04 seconds. These are observations, not guaranteed latency. Network conditions, Ollama Cloud availability and source websites determine fresh-answer speed.
...
Final regression checks: 116 backend tests passed. The focused frontend advisor/history checks passed (20 total), and the frontend production build passed with its existing bundle-size warning. The backend image was rebuilt and activated. Current generation uses Ollama Cloud and may be subject to account usage limits.
...
### Historical local latency baseline
The figures below are a historical local CPU baseline. They are not a prediction of Ollama Cloud latency, which depends on network conditions, provider capacity and selected usage plan.
...
### Current cloud-generation considerations
1. **No local generation-model retention**: Ollama Cloud owns model loading. Udaan does not keep a large generation model in local RAM or VRAM.
2. **Bounded context and output**: The service keeps prompt context and requested output short to reduce response time and cloud usage.
...
### Historical local-model benchmark
The previous small local model was compared with a smaller local alternative:
- **Smaller alternative**: While faster (total ~4–10s), it exhibited unacceptable regressions:
...
- **Current decision**: The prior local text model has been retired from generation. `gpt-oss:120b-cloud` is now the configured generation model.
...
- **Current limitation**: Ollama Cloud avoids local generation hardware requirements, but response time is not guaranteed and depends on the network, provider capacity and source retrieval.
